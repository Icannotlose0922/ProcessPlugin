...
bool AES::FileCipher(QFile *fin, QFile *fout)
{
...
bool AES::FileInvCipher(QFile *fin, QFile *fout)
{
	unsigned char input[16];
	unsigned char output[16];
	...
	if (!fin->open(QIODevice::ReadOnly))
	{
		LOG(ERROR) << " file is open error or write error:" << fin->errorString().toStdString().c_str();
		return false;
	}
	if (!fout->open(QIODevice::ReadWrite))
	{
